...
v4l2-ctl -d /dev/video2 --set-ctrl exposure_auto=1 # sets exposure to manual mode

for i in 5, 9, 10, 19, 39, 78, 156, 312, {313..1000}
do
  echo $i   
  v4l2-ctl -d /dev/video2 --set-ctrl exposure_absolute=$i #min=5 max=20000 step=1
  sleep 2s
done
...
#v4l2-ctl -d /dev/video2 --set-ctrl brightness=1 #min=30 max=255 step=1
#v4l2-ctl -d /dev/video2 --set-ctrl contrast=1 #min=0 max=10 step=1
...
#v4l2-ctl -d /dev/video2 --set-ctrl white_balance_temperature_auto=0 #need to disable this flag first
#v4l2-ctl -d /dev/video2 --set-ctrl white_balance_temperature=4500 #min=2800 max=10000 step=1
